...
export default function Dashboard() {
  ...
  useEffect(() => {
    ...
    // 1. Encomendas
    // Operador: vê geral do condomínio
    // Morador: filtra por unidadeIdNorm (+ blocoIdNorm, se existir)
    const encomendasRef = collection(firestore, "condominios", condominioId, "encomendas");

    (async () => {
      try {
        if (isOperator) {
          const qEncomendas = query(encomendasRef, where("status", "==", "AGUARDANDO"));
        ...
        const qEncomendas = query(encomendasRef, ...wheres);
        unsubs.push(onSnapshot(qEncomendas, (snap) => setEncomendasCount(snap.size)));
      } catch (e) {
        console.error("[Dashboard] erro ao montar query de encomendas:", e);
        setEncomendasCount(0);
      }
    })();
    ...
    unsubs.push(onSnapshot(qAssembleia, (snap) => {
      ...
      console.error("[Dashboard] snapshot error (qAssembleia):", err?.code || err, err);
