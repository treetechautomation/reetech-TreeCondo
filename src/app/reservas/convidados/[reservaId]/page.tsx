...
export default function ReservasConvidadosPage() {
  ...
  // fallback (se o hook devolver null em algum cenário)
  const fb = initializeFirebase() as any;
  const firestoreFinal = (firestore ?? fb?.firestore) as any;
const { session, isSessionLoading } = useSessionCtx();
  ...
  React.useEffect(() => {
    if (!firestore) return;
    if (!session?.activeCondominioId || !reservaId || !firestoreFinal) return;
    let alive = true;
