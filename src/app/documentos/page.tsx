...
export default function DocumentosPage() {
  ...
  async function openDoc(item: DocItem) {
    if (!item.storagePath || String(item.storagePath).trim() === "") {
      toast({ variant: "destructive", title: "Erro ao abrir", description: "Documento sem caminho no Storage (storagePath vazio). Reenvie o arquivo." });
      return;
    }
  ...
  async function downloadDoc(item: DocItem) {
    if (!item.storagePath || String(item.storagePath).trim() === "") {
      toast({ variant: "destructive", title: "Erro ao baixar", description: "Documento sem caminho no Storage (storagePath vazio). Reenvie o arquivo." });
      return;
    }
  ...
  async function handleDelete(item: DocItem) {
    if (!firestore || !condominioId || !canManage) return;
                  ...
                  <TabsContent key={cat} value={cat} className="mt-4">
                    <div className="w-full overflow-x-auto">
                      <div className="min-w-[720px]">
                        <div className="grid grid-cols-[1fr,160px,160px,120px,140px] px-3 py-2 text-xs font-medium text-muted-foreground border-b">
                        ...
                        {loading ? (
                          ...
                          list.map((it) => (
                            <div
                              key={it.id}
                              className="grid grid-cols-[1fr,160px,160px,120px,140px] items-center px-3 py-4 border-b last:border-b-0"
                            >
                              <div className="text-sm">{it.nome}</div>
                              <div className="text-right text-sm text-muted-foreground">{formatDateBR(it.publicacaoEm)}</div>
                              <div className="text-right text-sm text-muted-foreground truncate" title={it.createdByNome}>{it.createdByNome || "-"}</div>
                              ...
                              <div className="flex justify-end gap-2">
                                ...
                                {canManage && (
                                  <Button variant="destructive" size="icon" onClick={() => handleDelete(it)} title="Excluir"><Trash2 className="h-4 w-4" /></Button>
                                )}
                              </div>
                            </div>
                          ))
