/**
 * G1.7.5 — API de Catálogo Global de Recursos (Fundação) — Painel Global Treetech.
 *
 * GET  /api/global/recursos  → listagem paginada e ordenada.
 * POST /api/global/recursos  → cria GlobalRecurso (atômico com auditoria).
 *
 * Catálogo apenas: sem PATCH, DELETE, histórico, concessão/contratação
 * (globalRecursosContratados — entidade futura, decidida em G1.7.4, não
 * implementada aqui), consumo, limites, contadores, feature flags, cobrança,
 * upgrade/downgrade ou renovação. Sem relacionamento bidirecional — Produto
 * não passa a referenciar a lista de Recursos; o Recurso é a única entidade
 * que aponta para o Produto (produtoId). Mesmo padrão arquitetural de
 * /api/global/produtos e /api/global/planos:
 ...
 * - Auditoria atômica: criação do documento e do log (GLOBAL_RESOURCE_CREATED)
 *   em um único WriteBatch, via writeGlobalAuditLog() reutilizado.
 *
 * Validações do POST (Etapa 6): payload (`codigo`, `nome` obrigatórios,
 * `status` válido) → produto existe (404) → produto ATIVO (409) → sem outro
 * Recurso com o mesmo `codigo` dentro do mesmo `produtoId` (409, best effort
 * — mesma limitação estrutural documentada para Clientes/Empresas/Produtos/
 * Licenciamento/Planos/Assinaturas).
 */
...
import type {
  GlobalRecurso,
  GlobalRecursoStatus,
  CreateGlobalRecursoInput,
} from "@/types/global-recursos";

const STATUS_VALUES: GlobalRecursoStatus[] = ["ATIVO", "INATIVO"];
const DEFAULT_PAGE_SIZE = 20;
const MAX_PAGE_SIZE = 50;

function isValidStatus(v: string): v is GlobalRecursoStatus {
  return (STATUS_VALUES as string[]).includes(v);
}

function serialize(doc: FirebaseFirestore.DocumentSnapshot) {
  const data = (doc.data() || {}) as Partial<GlobalRecurso>;
  return {
    id: doc.id,
...
export async function GET(req: Request) {
  try {
    ...
    const db = adminDb();
    const col = db.collection("globalRecursos");
...
export async function POST(req: Request) {
  try {
    ...
    const body = (await req.json().catch(() => null)) as CreateGlobalRecursoInput | null;
    ...
    if (produtoData.status !== "ATIVO") {
      return jsonError("Produto não está ATIVO — não é possível criar recursos para um produto inativo.", 409);
    }

    // Sem outro Recurso com o mesmo codigo dentro do mesmo produto — BEST
    // EFFORT, mesma limitação estrutural documentada para Clientes/Empresas/
    // Produtos/Licenciamento/Planos/Assinaturas.
    const dupSnap = await db
      .collection("globalRecursos")
      .where("produtoId", "==", produtoId)
      .where("codigo", "==", codigo)
      .limit(1)
      .get();
    if (!dupSnap.empty) {
      return jsonError("Já existe um Recurso com este código para este Produto.", 409);
    }
    ...
    const afterSnapshot: Record<string, unknown> = { produtoId, codigo, nome, status };

    // Auditoria atômica: criação do recurso e do log GLOBAL_RESOURCE_CREATED
    // no mesmo WriteBatch (mesmo padrão de globalClientes/globalEmpresas/globalProdutos/globalLicencas/globalPlanos/globalAssinaturas).
    const recursoRef = db.collection("globalRecursos").doc();
    const batch = db.batch();
    batch.set(recursoRef, docData);
    await writeGlobalAuditLog(
      {
        actorUid: ctx.uid,
        actorEmail: ctx.email || undefined,
        action: "GLOBAL_RESOURCE_CREATED",
        entity: "GLOBAL_RESOURCE",
        entityId: recursoRef.id,
        before: null,
        after: afterSnapshot,
        source: "API",
      },
    ...
    const created = await recursoRef.get();
