...
export async function POST(req: Request) {
  ...
  try {
    ...
    const decoded = ctx.decodedToken;
    ...
    if (dateStr < todayInSaoPaulo()) {
      shadowEvaluate(db, motorDecision({
        ...
        blockOrigin: "DEFAULT",
      }), { condominioId, areaId, opcaoId, dateStr, uid: actorUid, actorUid, actorIsSuperAdmin, actorRole: "", priceCentavos: 0, isOperatorAction: false }).catch(() => {});
    ...
    if (targetUidBody && targetUidBody !== uid) {
      ...
      const isOperador = isOperatorRole(actorRole) || actorIsSuperAdmin;
    ...
    // ── UN.6C.1: Resolve canonical unit via VinculoUnidade ──
    const mdPre = ctx.membroData || {};
    ...
    if (area.escopoReserva === "BLOCO" && Array.isArray(area.blocosPermitidos) && area.blocosPermitidos.length > 0) {
      ...
      if (!membroBlocoNorm || !area.blocosPermitidos.includes(membroBlocoNorm)) {
        shadowEvaluate(db, motorDecision({
          ...
          blockOrigin: "AREA",
        }), { condominioId, areaId, opcaoId, dateStr, uid, actorUid, actorIsSuperAdmin, actorRole: "", priceCentavos: valorCobrado, isOperatorAction: uid !== actorUid }).catch(() => {});
    ...
    } catch { /* policyVersion permanece 0 */ }
      ...
      result = await db.runTransaction(async (tx: any) => {
        ...
        if (isCompound) {
          ...
          if (hasFee) {
            ...
            const mdFin = membroSnap.exists ? (membroSnap.data() || {}) : {};
            const unidadeIdRaw = reservaUnidadeId || String(mdFin.unidadeId || mdFin.apto || "");
            const unidadeIdNormVal = reservaUnidadeIdNormVal || String(mdFin.unidadeIdNorm || normUnidade(unidadeIdRaw) || "");
            ...
            tx.set(cobrancaRef, {
              ...
              numeroReserva,
              moradorUid: uid,
              moradorNome: String(mdFin.nome || ""),
              blocoId: reservaBlocoId || String(mdFin.blocoId || mdFin.bloco || ""),
              blocoIdNorm: reservaBlocoIdNormVal || String(mdFin.blocoIdNorm || normBloco(mdFin.blocoId || mdFin.bloco) || ""),
              blocoNome: reservaBlocoNombre || blocoNome || String(mdFin.blocoId || mdFin.bloco || ""),
        ...
        if (opcaoId === "com_campo") {
          ...
          const reservaRef = reservasCol.doc();
          const dt = isoNoonUTC(dateStr);
          const statusFinalCampo = hasFee ? "PENDENTE_PAGAMENTO" : "APROVADA";
          tx.set(reservaRef, {
            areaId, condominioId, uid, criadoPorUid: actorUid,
            reservaManualPorOperador: uid !== actorUid,
            status: statusFinalCampo, precisaAprovacao: !hasFee,
        ...
        if (hasFee) {
          const cobrancaRef = db.collection("condominios").doc(condominioId).collection("financeiro").doc();
          const mdFin = membroSnap.exists ? (membroSnap.data() || {}) : {};
          const unidadeIdRaw = canonicalUnidadeNumero || String(mdFin.unidadeId || mdFin.apto || "");
          const unidadeIdNormVal = canonicalUnidadeIdNorm || String(mdFin.unidadeIdNorm || normUnidade(unidadeIdRaw) || "");
          const numeroReserva = dateStr.replace(/-/g, "") + "-" + reservaRef.id.substring(0, 6).toUpperCase();
          const competenciaVal = dateStr.substring(0, 7);
          ...
          tx.set(cobrancaRef, {
            ...
            moradorUid: uid,
            moradorNome: String(mdFin.nome || ""),
            blocoId: canonicalBlocoId || String(mdFin.blocoId || mdFin.bloco || ""),
            blocoIdNorm: canonicalBlocoIdNorm || String(mdFin.blocoIdNorm || normBloco(mdFin.blocoId || mdFin.bloco) || ""),
            blocoNome: canonicalBlocoNome || blocoNome || String(mdFin.blocoId || mdFin.bloco || ""),
            unidadeId: unidadeIdRaw,
            unidadeIdNorm: unidadeIdNormVal,
            unidadeNome: unidadeNomeVal,
            areaId,
    ...
    if ((result as any)?.mode === "FILA") {
      ...
      }), { condominioId, areaId, opcaoId, dateStr, uid, actorUid, actorIsSuperAdmin: false, actorRole: "", priceCentavos: valorCobrado, isOperatorAction: false }).catch(() => {});
    }
