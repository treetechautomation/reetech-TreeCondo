...
export default function AcessoPage() {
  ...
  React.useEffect(() => {
    ...
    const qy = canOperarPortaria
      ...
      : query(col, where("moradorUid", "==", uid), orderBy("createdAt", "desc"));
    ...
    return () => unsub();
  }, [firestore, condominioId, uid, canOperarPortaria]);

  const filtrados = React.useMemo(() => itens.filter((i) => i.status === tab), [itens, tab]);
  ...
  async function criarAcesso() {
    ...
    try {
      ...
      setOpenNew(false);
      // Reset form
      setNome(""); setTipo("VISITANTE"); setTelefone(""); setDocumento(""); setPlaca(""); setEmpresa(""); setObservacao(""); setJanelaInicio(dtLocalNowPlus(0)); setJanelaFim(dtLocalNowPlus(4));
    } catch (e: any) {
      console.error(e);
          ...
          {!condominioId ? <div className="p-4 text-sm text-muted-foreground">Selecione um condomínio para ver/criar acessos.</div> :
            <Tabs value={tab} onValueChange={(v) => setTab(v as StatusAcesso)}>
              <TabsList>
                ...
                <TabsTrigger value="SAIU">Histórico</TabsTrigger>
              </TabsList>
              {(["PENDENTE", "AUTORIZADO", "ENTROU", "SAIU", "NEGADO", "CANCELADO"] as StatusAcesso[]).map((status) => {
                const list = itens.filter((x) => x.status === status);
                return (
                  <TabsContent key={status} value={status} className="mt-4">
                    <div className="w-full overflow-x-auto">
                      <div className="min-w-[900px]">
                        <div className="grid grid-cols-[1fr,160px,160px,220px] px-3 py-2 text-xs font-medium text-muted-foreground border-b">
                            <div>Nome</div>
                            <div className="text-right">Janela Início</div>
                            <div className="text-right">Janela Fim</div>
                            <div className="text-right">Ações</div>
                        </div>
                        {loading ? <div className="p-4 text-sm text-muted-foreground">Carregando...</div> : list.length === 0 ? <div className="p-4 text-sm text-muted-foreground">Nada aqui ainda.</div> :
                          list.map((it) => (
                            <div key={it.id} className="grid grid-cols-[1fr,160px,160px,220px] items-center px-3 py-4 border-b last:border-b-0">
                              ...
                              <div className="flex justify-end gap-2">
                                <Button variant="outline" size="icon" title="Detalhes" onClick={() => setDetailedItem(it)}><Eye className="h-4 w-4" /></Button>
                                {canOperarPortaria && it.status === "PENDENTE" && <>
                                    <Button size="sm" onClick={() => atualizarStatusAcesso(it, "AUTORIZADO")} title="Autorizar"><Check className="h-4 w-4" /></Button>
                                    <Button size="sm" variant="destructive" onClick={() => atualizarStatusAcesso(it, "NEGADO")} title="Negar"><Ban className="h-4 w-4" /></Button>
                                </>}
                                {canOperarPortaria && it.status === "AUTORIZADO" && <Button size="sm" onClick={() => atualizarStatusAcesso(it, "ENTROU")} title="Liberar entrada"><LogIn className="mr-2 h-4 w-4" />Entrada</Button>}
                                {canOperarPortaria && it.status === "ENTROU" && <Button size="sm" variant="outline" onClick={() => atualizarStatusAcesso(it, "SAIU")} title="Liberar saída"><LogOut className="mr-2 h-4 w-4" />Saída</Button>}
                                {isMorador && it.moradorUid === uid && it.status === 'PENDENTE' && <Button size="sm" variant="destructive" onClick={() => handleDelete(it.id)} title="Excluir Solicitação"><Trash2 className="h-4 w-4" /></Button>}
                              </div>
                            </div>
                          ))
                        }
