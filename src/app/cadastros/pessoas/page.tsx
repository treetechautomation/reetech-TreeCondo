      ...
      try {
        ...
        if (session.superAdmin) {
          ...
          if (vinculosAtivos.length > 0) {
            ...
            list = snaps
              .map((snap: any) => {
                const data = (snap.data?.() ?? snap.data ?? {}) as any;
                const cid = String(data.condominioId ?? snap.id ?? "").trim();
                return {
                  id: String(snap.id),
                  condominioId: cid || String(snap.id),
