...
/** Mock temporário (para não quebrar UI dos tabs ainda não ligados no Firestore) */
...
export default function CadastrosPage() {
  ...
  /** Forms */
  const {
    register: registerMorador,
    handleSubmit: handleSubmitMorador,
    formState: { errors: errorsMorador, isSubmitting: isSubmittingMorador },
    reset: resetMorador,
  } = useForm<MoradorFormData>({
    resolver: zodResolver(moradorSchema),
    defaultValues: { isResponsavel: false },
  });
  ...
  const {
    ...
    reset: resetFuncionario,
  } = useForm<FuncionarioFormData>({ resolver: zodResolver(funcionarioSchema) });
  ...
  const {
    ...
    reset: resetVeiculo,
  } = useForm<VeiculoFormData>({ resolver: zodResolver(veiculoSchema) });

  const {
    register: registerFornecedor,
    handleSubmit: handleSubmitFornecedor,
    formState: { errors: errorsFornecedor },
    reset: resetFornecedor,
  } = useForm<FornecedorFormData>({ resolver: zodResolver(fornecedorSchema) });

  const {
    control: controlPet,
    register: registerPet,
    handleSubmit: handleSubmitPet,
    formState: { errors: errorsPet },
    reset: resetPet,
  } = useForm<PetFormData>({ resolver: zodResolver(petSchema) });

  const {
    register: registerBloco,
    handleSubmit: handleSubmitBloco,
    formState: { errors: errorsBloco },
    reset: resetBloco,
  } = useForm<BlocoFormData>({ resolver: zodResolver(blocoSchema) });

  /** Submit Morador (Firestore real) */
  const onMoradorSubmit = async (data: MoradorFormData) => {
    if (!moradoresRef) {
      toast({
        title: "Erro",
        description: "Selecione um condomínio, bloco e unidade para continuar.",
        variant: "destructive",
      });
      return;
    }
  ...
  /** Outros submits (mock por enquanto) */
  const onFuncionarioSubmit = (data: FuncionarioFormData) => {
    console.log("FUNCIONARIO:", data);
    setOpenFuncionario(false);
    resetFuncionario();
  };

  const onVeiculoSubmit = (data: VeiculoFormData) => {
    console.log("VEICULO:", data);
    setOpenVeiculo(false);
    resetVeiculo();
  };

  const onFornecedorSubmit = (data: FornecedorFormData) => {
  ...
  const onBlocoSubmit = (data: BlocoFormData) => {
  ...
  const headerHint = (
    <div className="text-xs text-muted-foreground">
      {condominioAtivoId ? (
        <>
          Condomínio: <span className="font-medium">{condominioAtivoId}</span>
          {blocoAtivoId ? (
            <>
              {" "}• Bloco: <span className="font-medium">{blocoAtivoId}</span>
            </>
          ) : null}
          {unidadeAtivaId ? (
            <>
              {" "}• Unidade: <span className="font-medium">{unidadeAtivaId}</span>
            </>
          ) : null}
        </>
      ) : (
        "Selecione um condomínio no topo para começar."
      )}
    </div>
  );

  return (
    <AppLayout pageTitle="Gestão de Cadastro" headerActions={headerHint}>
      <Tabs defaultValue="moradores">
        <TabsList className="mb-4">
          <TabsTrigger value="moradores">
            <User className="mr-2" />
            Moradores
          </TabsTrigger>
          ...
          <div className="flex justify-end mb-4">
            <Dialog open={openMorador} onOpenChange={setOpenMorador}>
              <DialogTrigger asChild>
                <Button disabled={!unidadeAtivaId}>
              ...
              <DialogContent>
                ...
                <form onSubmit={handleSubmitMorador(onMoradorSubmit)}>
                  <div className="space-y-4 py-4">
                    <div className="space-y-1">
                      ...
                      <Input
                        ...
                        {...registerMorador("nome")}
                      />
                      {errorsMorador.nome && (
                        <p className="text-xs text-destructive">{errorsMorador.nome.message}</p>
                      )}
                    </div>
                    ...
                    <div className="space-y-1">
                      ...
                      <Input
                        ...
                        {...registerMorador("email")}
                      />
                      {errorsMorador.email && (
                        <p className="text-xs text-destructive">{errorsMorador.email.message}</p>
                      )}
                    </div>
                  ...
                  <DialogFooter>
                    <Button type="submit" disabled={isSubmittingMorador}>
                      {isSubmittingMorador ? "Salvando..." : "Salvar"}
                    </Button>
                  </DialogFooter>
                </form>
              </DialogContent>
              ...
              {isLoading ? (
                ...
                <TableRow>
                  <TableCell colSpan={4} className="text-center text-muted-foreground">
                    Selecione um condomínio, bloco e unidade para ver os moradores.
                  </TableCell>
                </TableRow>
              ) : (moradores?.length ?? 0) === 0 ? (
                <TableRow>
                  <TableCell colSpan={4} className="text-center text-muted-foreground">
              ...
              <DialogContent>
                ...
                <form onSubmit={handleSubmitFuncionario(onFuncionarioSubmit)}>
                  <div className="grid gap-4 py-4">
                    <div className="space-y-1">
                      <Label>Nome</Label>
                      <Input placeholder="Nome completo" {...registerFuncionario("nome")} />
                      {errorsFuncionario.nome && (
                        <p className="text-xs text-destructive">{errorsFuncionario.nome.message}</p>
                      )}
                    </div>

                    <div className="space-y-1">
                      <Label>Cargo</Label>
                      <Input placeholder="Ex: Zelador" {...registerFuncionario("cargo")} />
                      {errorsFuncionario.cargo && (
                        <p className="text-xs text-destructive">{errorsFuncionario.cargo.message}</p>
                      )}
                    </div>

                    <div className="space-y-1">
                      <Label>Horário</Label>
                      <Input placeholder="Ex: 08:00 - 17:00" {...registerFuncionario("horario")} />
                      {errorsFuncionario.horario && (
                        <p className="text-xs text-destructive">{errorsFuncionario.horario.message}</p>
                      )}
                    </div>

                    <div className="space-y-1">
                      <Label>Contato</Label>
                      <Input placeholder="(99) 99999-9999" {...registerFuncionario("contato")} />
                      {errorsFuncionario.contato && (
                        <p className="text-xs text-destructive">{errorsFuncionario.contato.message}</p>
                      )}
                    </div>
                  </div>
              ...
              <DialogContent>
                ...
                <form onSubmit={handleSubmitVeiculo(onVeiculoSubmit)}>
                  <div className="grid gap-4 py-4">
                    <div className="space-y-1">
                      <Label>Placa</Label>
                      <Input placeholder="ABC-1234" {...registerVeiculo("placa")} />
                      {errorsVeiculo.placa && (
                        <p className="text-xs text-destructive">{errorsVeiculo.placa.message}</p>
                      )}
                    </div>

                    <div className="space-y-1">
                      <Label>Modelo</Label>
                      <Input placeholder="Ex: Honda Civic" {...registerVeiculo("modelo")} />
                      {errorsVeiculo.modelo && (
                        <p className="text-xs text-destructive">{errorsVeiculo.modelo.message}</p>
                      )}
                    </div>

                    <div className="space-y-1">
                      <Label>Unidade</Label>
                      <Input placeholder="Ex: Apto 101" {...registerVeiculo("unidade")} />
                      {errorsVeiculo.unidade && (
                        <p className="text-xs text-destructive">{errorsVeiculo.unidade.message}</p>
                      )}
                    </div>
                  </div>
              ...
              <DialogContent>
                ...
                <form onSubmit={handleSubmitFornecedor(onFornecedorSubmit)}>
                  <div className="grid gap-4 py-4">
                    <div className="space-y-1">
                      <Label>Nome</Label>
                      <Input placeholder="Nome da empresa" {...registerFornecedor("nome")} />
                      {errorsFornecedor.nome && (
                        <p className="text-xs text-destructive">{errorsFornecedor.nome.message}</p>
                      )}
                    </div>

                    <div className="space-y-1">
                      <Label>Serviço</Label>
                      <Input placeholder="Ex: Jardinagem" {...registerFornecedor("servico")} />
                      {errorsFornecedor.servico && (
                        <p className="text-xs text-destructive">{errorsFornecedor.servico.message}</p>
                      )}
                    </div>

                    <div className="space-y-1">
                      <Label>Contato</Label>
                      <Input placeholder="Email ou telefone" {...registerFornecedor("contato")} />
                      {errorsFornecedor.contato && (
                        <p className="text-xs text-destructive">{errorsFornecedor.contato.message}</p>
                      )}
                    </div>
                  </div>
              ...
              <DialogContent>
                ...
                <form onSubmit={handleSubmitPet(onPetSubmit)}>
                  <div className="grid gap-4 py-4">
                    <div className="space-y-1">
                      <Label>Nome</Label>
                      <Input placeholder="Nome do animal" {...registerPet("nome")} />
                      {errorsPet.nome && (
                        <p className="text-xs text-destructive">{errorsPet.nome.message}</p>
                      )}
                    </div>

                    <div className="space-y-1">
                      <Label>Raça</Label>
                      <Input placeholder="Ex: Labrador" {...registerPet("raca")} />
                      {errorsPet.raca && (
                        <p className="text-xs text-destructive">{errorsPet.raca.message}</p>
                      )}
                    </div>
                    ...
                    <div className="space-y-1">
                      ...
                      />
                      {errorsPet.porte && (
                        <p className="text-xs text-destructive">{errorsPet.porte.message}</p>
                      )}
                    </div>

                    <div className="space-y-1">
                      <Label>Unidade</Label>
                      <Input placeholder="Ex: Apto 101" {...registerPet("unidade")} />
                      {errorsPet.unidade && (
                        <p className="text-xs text-destructive">{errorsPet.unidade.message}</p>
                      )}
                    </div>
                  </div>
              ...
              <DialogContent>
                ...
                <form onSubmit={handleSubmitBloco(onBlocoSubmit)}>
                  <div className="grid gap-4 py-4">
                    <div className="space-y-1">
                      <Label>Nome</Label>
                      <Input placeholder="Ex: Bloco A" {...registerBloco("nome")} />
                      {errorsBloco.nome && (
                        <p className="text-xs text-destructive">{errorsBloco.nome.message}</p>
                      )}
                    </div>

                    <div className="space-y-1">
                      <Label>Nº de Unidades</Label>
                      <Input type="number" placeholder="Ex: 40" {...registerBloco("unidades")} />
                      {errorsBloco.unidades && (
                        <p className="text-xs text-destructive">{errorsBloco.unidades.message}</p>
                      )}
                    </div>
                  </div>
