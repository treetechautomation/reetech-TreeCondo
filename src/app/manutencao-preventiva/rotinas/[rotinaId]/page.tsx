...
type Execucao = {
  ...
  status: string;
  dataProgramada?: any;
  dataConcluida?: any; // legacy
  dataExecutadaEm?: any; // new
  proximaProgramadaEm?: any; // new
...
export default function RotinaDetalhePage() {
  ...
  const handleConcluir = async (execucao: Execucao) => {
    ...
    if (String(execucao.status || "").toUpperCase() === "CONCLUIDA") {
      toast({ title: "Esta execução já foi concluída." });
      return;
    }

    try {
      const dataBase = execucao.dataProgramada?.toDate?.() ?? new Date();
      let proximaData = calculateNextExecution(dataBase, rotina.recorrenciaTipo);

      while (proximaData.getTime() <= dataBase.getTime()) {
        proximaData = calculateNextExecution(proximaData, rotina.recorrenciaTipo);
      }
      proximaData.setHours(0, 0, 0, 0);

      const execucoesCollectionRef = collection(firestore, `condominios/${condominioId}/manutencaoExecucoes`);
      const rotinaRef = doc(firestore, `condominios/${condominioId}/manutencaoRotinas`, rotina.id);
      const execucaoRef = doc(execucoesCollectionRef, execucao.id);

      const startOfNextDay = startOfDay(proximaData);
      const endOfNextDay = new Date(startOfNextDay.getTime() + 24 * 60 * 60 * 1000);

      const qDup = query(
        execucoesCollectionRef,
        where("rotinaId", "==", rotina.id),
        where("status", "==", "PROGRAMADA"),
        where("dataProgramada", ">=", Timestamp.fromDate(startOfNextDay)),
        where("dataProgramada", "<", Timestamp.fromDate(endOfNextDay))
      );

      const dupSnap = await getDocs(qDup);
      const hasDuplicate = !dupSnap.empty;

      const batch = writeBatch(firestore);

      batch.update(execucaoRef, {
        status: "CONCLUIDA",
        dataConcluida: serverTimestamp(),
        dataExecutadaEm: serverTimestamp(),
        proximaProgramadaEm: Timestamp.fromDate(proximaData),
      });

      batch.update(rotinaRef, {
        ultimaExecucaoEm: serverTimestamp(),
        proximaExecucaoEm: Timestamp.fromDate(proximaData),
      ...
      if (!hasDuplicate) {
        ...
        const proximaExecRef = doc(execucoesCollectionRef);
        batch.set(proximaExecRef, proximaExecPayload);
      }

      await batch.commit();

      toast({ title: hasDuplicate ? "Execução concluída. Próxima já existia." : "Execução concluída e próxima agendada!" });
